...
_github_link: str = (
    f"https://github.com/{_data['github']['user_name']}/{_data['github']['repo_name']}"
)
_author_names_in_order: List[str] = [
    _author["name"]
    for _author in (_data["contact"]["authors"] + _data["contact"]["corresponding_authors"])
]
_announcement = _data["docs"]["announcement"]

_url_copyright = (
    f"{_github_link}/blob/{_data['github']['branch_name']}/{_data['copyright']['path']}"
)
...
copyright: str = (
    ...
    if _copyright_start_year != datetime.date.today().year
    else f"{_copyright_start_year}, "
...
def _add_icon_link(
    name: str, url: str, icon: str, type: Literal["fontawesome", "url", "local"] = "fontawesome"
) -> NoReturn:
...
man_pages: List[Tuple[str, str, str, Union[str, List[str]], str]] = [
    (root_doc, _package_name, f"{_project_name} Documentation", _author_names_in_order, "1")
]
